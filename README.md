...
- если переводчик обрабатывает запросы долго, увеличьте `NEWS_TRANSLATOR_READ_TIMEOUT_SECONDS` до `900` или выше
- перевод новостей идет отдельной очередью: crawl сначала сохраняет оригинал, затем `NewsTranslatePendingArticlesJob` переводит статьи по одной
...
- При старте `Sidekiq` автоматически запускается recovery очереди перевода: очищается stale `news:translation:pending_articles_lock`, свежие `failed` новости переводятся обратно в `pending`, затем снова ставится `NewsTranslatePendingArticlesJob`.
